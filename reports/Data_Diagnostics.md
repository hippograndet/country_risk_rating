...
This report details the data quality assessment and feature selection process for the OECD country risk rating prediction project. The dataset combines OECD sovereign risk ratings with 94 World Bank macroeconomic indicators across 6 economic dimensions. The preprocessing pipeline addresses missingness, stability, and redundancy to produce a robust feature set for modeling.

## Data Quality

### Missingness Patterns

Missing values are pervasive across the dataset, with indicator-level missingness ranging from 0% to 99%. The distribution of missingness across features is highly skewed:
...
1. **Missingness Filter**: Excludes features with >50% missing values
2. **Variance Filter**: Removes low-variance features (threshold: 0.01)
3. **Correlation Filter**: Eliminates features with pairwise correlation >0.95
4. **Mutual Information**: Retains top 50 features most informative about the target

The pipeline reduces the feature space from 94 raw indicators to 63 engineered features:

![Feature Selection Waterfall](plots/feature_selection_waterfall.png)

### Engineered Features

Feature engineering creates domain-informed transformations that improve predictive signal:

- **Growth rates**: Year-over-year changes in key indicators
- **Ratios**: Debt-to-GDP, current account balance, fiscal balance
- **Averages**: 3-year and 5-year moving averages to smooth volatility
- **Z-scores**: Standardized indicators to enable cross-country comparison
...
The final dataset after preprocessing contains:
- **Observations**: 4,081 country-year pairs
- **Features**: 63 engineered variables
- **Target**: OECD risk ratings (1–7 scale)
- **Temporal range**: 1999–2024

The dataset overview shows balanced representation across rating categories after temporal splitting:
...
- **Explore alternative missingness imputation strategies** beyond median imputation, such as multiple imputation or model-based approaches.
